...
static void PollingToSendIBIRequest(void)
{
    ...
    if((UART0->FIFOSTS&UART_FIFOSTS_RXEMPTY_Msk) == 0U)
    {
        ...
        do {
            if(((chTrgIBI == 'i') || (chTrgIBI == 'I')) && !I3CS_IS_DA_VALID(I3CS0))
            {
            ...
            if(I3CS_SendIBIRequest(I3CS0, IBI_MDB, IBI_PL, IBI_PLLen) != I3CS_STS_NO_ERR)
...
int32_t I3CS_ProcessNoneRespInt(I3CS_T *i3cs, uint32_t u32IntMask)
{
    ...
    if(u32IntMask & I3CS_INTEN_IBI_UPDATED)
    {
        printf("[ IBI Request status ]\n");
        if((i3cs->SIRRESP&I3CS_SIRRESP_IBISTS_Msk) == I3CS_IBI_ACCEPTED)
        {
            /* IBI accepted by the Master */
            printf("\t* IBI accepted by the Master\n\n");
        }
        else if((i3cs->SIRRESP&I3CS_SIRRESP_IBISTS_Msk) == I3CS_IBI_MASTER_TERMINATE)
        {
            /* IBI master early terminate */
            printf("\t* The Master early terminate\n\n");
            /* Clear CmdQ, Tx buffer */
            I3CS_ResetAndResume(i3cs, (I3CS_RESET_CMD_QUEUE | I3CS_RESET_TX_BUF), FALSE);
        }
        else if((i3cs->SIRRESP&I3CS_SIRRESP_IBISTS_Msk) == I3CS_IBI_NOT_ATTEMPTED)
        {
            /* IBI NOT attempted */
...
int main(void)
{
    ...
    /* Configure I3CS0 default settings */
