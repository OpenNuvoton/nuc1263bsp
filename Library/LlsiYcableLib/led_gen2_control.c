...
{
    /* Set unused BPWM to count Gen2 LED feedback waveform length */
...
void LED_Gen2_Transfer_Data(uint8_t target_port)
{
    ...
    PDMA_SetTransferMode(target_port, 36 + target_port, TRUE, (uint32_t)&LED_Gen2_PDMA_DESC[target_port][0]);    // PDMA_LLSI0 = 36
}
...
void LED_Gen2_Control_CMD_ID(uint8_t cmd, uint8_t id)
{
    ...
    *Gen2_Ctrl.fPDMA_Done = 0;
    ...
    /* Start LLSI PDMA transfer */
    PDMA_SetTransferMode(Gen2_Ctrl.Current_Port, 36 + Gen2_Ctrl.Current_Port, TRUE, (uint32_t)&LED_Gen2_PDMA_DESC[Gen2_Ctrl.Current_Port][0]);    // PDMA_LLSI0 = 36
...
void LED_Gen2_LLSI_PDMA_Init(uint8_t target_port)
{
    ...
    /* Change PDMA setting */
    PDMA_SetTransferMode(target_port, 36 + target_port, TRUE, 0);    // PDMA_LLSI0 = 36
    PDMA->DSCT[target_port].NEXT = (uint32_t)&LED_Gen2_PDMA_DESC[target_port][0] - (PDMA->SCATBA);
