...
{StripALEDData, StripBLEDData,
 FAN1LEDData, FAN2LEDData, FAN3LEDData, FAN4LEDData};
const uint32_t LED_Data_Array_Size[TOTAL_LED_AREA] = 
{LED_STRIPA_LEN, LED_STRIPB_LEN,
 LED_FAN1LED_LEN, LED_FAN2LED_LEN, LED_FAN3LED_LEN, LED_FAN4LED_LEN};
...
volatile LLSI_T *LLSI_Port_Mapping[TOTAL_LED_AREA] = {LLSI0, LLSI1, LLSI2, LLSI3, LLSI4, LLSI5};                         // The sequence should match to number of LLSI_Num

/* LLSI port GPIO setting table */
const LED_LLSI_IO_SETTING LED_LLSI_IO_Setting[TOTAL_LED_AREA] =
    {{PB, 15, BIT15, GPAB_IRQn,  0x3C /* SYS->GPB_MFPH */, 0x7C, SYS_GPB_MFPH_PB15MFP_Msk, SYS_GPB_MFPH_PB15MFP_LLSI0_OUT},
     {PB, 14, BIT14, GPAB_IRQn,  0x3C /* SYS->GPB_MFPH */, 0x78, SYS_GPB_MFPH_PB14MFP_Msk, SYS_GPB_MFPH_PB14MFP_LLSI1_OUT},
     {PC, 3,  BIT3,  GPCDF_IRQn, 0x40 /* SYS->GPC_MFPL */, 0x8C, SYS_GPC_MFPL_PC3MFP_Msk,  SYS_GPC_MFPL_PC3MFP_LLSI2_OUT},
     {PC, 2,  BIT2,  GPCDF_IRQn, 0x40 /* SYS->GPC_MFPL */, 0x88, SYS_GPC_MFPL_PC2MFP_Msk,  SYS_GPC_MFPL_PC2MFP_LLSI3_OUT},
     {PB, 5,  BIT5,  GPAB_IRQn,  0x38 /* SYS->GPB_MFPL */, 0x54, SYS_GPB_MFPL_PB5MFP_Msk,  SYS_GPB_MFPL_PB5MFP_LLSI4_OUT},
     {PB, 4,  BIT4,  GPAB_IRQn,  0x38 /* SYS->GPB_MFPL */, 0x50, SYS_GPB_MFPL_PB4MFP_Msk,  SYS_GPB_MFPL_PB4MFP_LLSI5_OUT}};
...
volatile LED_Setting_T *LED_Mapping[TOTAL_LED_AREA] = {&StripA_LEDSetting, &StripB_LEDSetting, &FAN1_LEDSetting, &FAN2_LEDSetting, &FAN3_LEDSetting,
                                                       &FAN4_LEDSetting};    // The sequence should match to number of LLSI_Num
...
void LLSI_Initial(uint8_t port)
{
    ...
    *(uint32_t *)(SYS_BASE + LED_LLSI_IO_Setting[port].IO_MFP_OFFSET) =
    ...
    if(PDMA_Mapping[port]->LED_Type == Type_RGB)
        LLSI_SET_RGB_FORMAT((LLSI_T *)(APB1_BASE + 0x100000*(PDMA_Mapping[port]->LLSI_Num%2) + 0x54000 + 0x200*(PDMA_Mapping[port]->LLSI_Num/2)));
...
void PDMA_Initial(uint8_t port)
{
    ...
    PDMA_SetBurstType(port, PDMA_REQ_SINGLE, 0);

    /* Set source address, destination address */
    PDMA_SetTransferAddr(port, (uint32_t)LED_Mapping[port]->LED_Data, PDMA_SAR_INC, (uint32_t)&(LLSI_Port_Mapping[port]->DATA), PDMA_DAR_FIX);
...
{
    LLSI_T * target_LLSI = (LLSI_T *)(LLSI0_BASE + 0x100000*(target_port & 0x1) + 0x200*(target_port >> 1));
    ...
    if(target_LLSI->STATUS & LLSI_STATUS_UNDFLIF_Msk)
    {
        ...
        PDMA->RESET = 1 << (target_port);
        while(PDMA->CHCTL & (1 << target_port));
        PDMA_SetREQSEL(target_port);
        PDMA->DSCT[target_port].CTL &= ~PDMA_DSCT_CTL_OPMODE_Msk;
        ...
        if(LED_GEN2_MAX_SUPPORT_PORT > target_port)
        {
            if(LED_Gen2_Port_Setting[target_port].Use_Gen2 == TRUE)
            {
                ...
                PDMA->DSCT[target_port].NEXT = (uint32_t)&LED_Gen2_PDMA_DESC[target_port][0] - (PDMA->SCATBA);

                /* Even descriptor table configuration */
                for(i = 0; i < (LED_GEN2_MAX_STRIP_COUNT*LED_GEN2_PDMA_DESC_NUM); i+=2)
                    /* Configure next descriptor table address */
                    LED_Gen2_PDMA_DESC[target_port][i].NEXT = (uint32_t)&LED_Gen2_PDMA_DESC[target_port][i+1] - (PDMA->SCATBA); /* next descriptor table is table i+1 */
            ...
            {
                /* Transfer type is single transfer */
                PDMA_SetBurstType(target_port, PDMA_REQ_SINGLE, 0);

                /* Set source address, destination address */
                PDMA_SetTransferAddr(target_port, (uint32_t)PDMA_Mapping[target_port]->LED_Data, PDMA_SAR_INC, (uint32_t)&(target_LLSI->DATA), PDMA_DAR_FIX);
            }
        }
        /* Eanble PDMA */
        PDMA->CHCTL |= (1 << target_port);
